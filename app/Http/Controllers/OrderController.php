...
class OrderController extends Controller
{
    ...
    {
        $packages = Products_Service::where(
            'type',
            'hosting'
        )->get();
    ...
    public function store(Request $request): ?RedirectResponse
    {
        ...
        $convertedPrice = $billingService->convertCurrency(
            $package->price,
            $package->currency,
            $request->currency
        );
        ...
        $paymentData = [
            'invoice_id' => $invoice->id,
            'payment_gateway_id' => $request->payment_method === 'credit_card' ? 2 : 1,
            // Assuming 2 is Stripe and 1 is PayPal
